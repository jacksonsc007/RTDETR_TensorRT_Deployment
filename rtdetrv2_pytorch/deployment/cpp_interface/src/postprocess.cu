...
__global__ void decode_kernel(float* src, float* dst, int numBboxes, int numClasses, float confThresh, int maxObjects, int numBoxElement){
    ...
    int index = (int)atomicAdd(dst, 1); // The size of dst is (1+numboxes * 7). The additional one is used to record the number of valid boxes.
    if (index >= maxObjects) return; // keep at most maxObjects boxes
