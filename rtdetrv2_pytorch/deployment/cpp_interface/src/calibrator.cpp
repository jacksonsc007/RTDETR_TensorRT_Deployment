    ...
    , read_cache_(read_cache)
{
    ...
    // the original images acts as the second input.
    int64_t orig_img_size[2] = {input_w, input_h};
    CHECK( cudaMalloc(&device_input_orig_image_size, sizeof(int64_t) * 2) );
    CHECK( cudaMemcpy(device_input_orig_image_size, orig_img_size, sizeof(int64_t) * 2, cudaMemcpyHostToDevice));
...
Int8EntropyCalibrator2::~Int8EntropyCalibrator2()
{
    ...
    CHECK( cudaFree(device_input_orig_image_size) );
    if (batch_data) {
        delete[] batch_data;
    }
}
...
bool Int8EntropyCalibrator2::getBatch(void* bindings[], const char* names[], int nbBindings) noexcept
{
    ...
    CHECK( cudaMemcpy(device_input_, batch_data, input_count_ * sizeof(float), cudaMemcpyHostToDevice) );
    bindings[0] = device_input_;
    bindings[1] = device_input_orig_image_size;
    return true;
}
...
const void* Int8EntropyCalibrator2::readCalibrationCache(size_t& length) noexcept
{
    ...
    {
        printf("\e[31m[INFO]\e[m Found calibration cache: %s\n", calib_table_name_.c_str());
...
{
    printf("\e[31m[INFO]\e[m writing calib cache: %s size: %zu\n", calib_table_name_.c_str(), length);
