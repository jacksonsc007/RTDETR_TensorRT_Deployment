/*
This is the first successful implementation of RT-DETR engine with custom plugins.
We discover that the loaded plugin lib should be persistent in memory, or segementation fault occurs when
creating execution context, running enqueue during inference.

In implementation 3, we determine that the segmentation fault is caused by the plugin lib not being loaded for runtime.
*/
...
int32_t RTDetr::get_engine(){
    ...
    */
    ...
    static LibraryPtr nvinferPluginLib{};
    nvinferPluginLib = samplesCommon::loadLibrary(pluginLibs.c_str());
    static auto pInitLibNvinferPlugins
