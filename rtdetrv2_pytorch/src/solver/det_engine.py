...
def evaluate_trt_engine(model: TRTInference, criterion: torch.nn.Module, postprocessor, data_loader, coco_evaluator: CocoEvaluator, device):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 1000, header):
        ...
        for i in range(batch_size):
            # NOTE: remap the class id to coco category id
...
@torch.no_grad()
def evaluate_onnx(model: InferenceSession, criterion: torch.nn.Module, postprocessor, data_loader, coco_evaluator: CocoEvaluator, device):
    # model.eval()
    criterion.eval()
    coco_evaluator.cleanup()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 1000, header):
        ...
        output = model.run(
            ...
            output_names=None,
            input_feed={'images': samples.cpu().numpy(), "orig_target_sizes": orig_target_sizes.cpu().numpy()}
        )
        ...
        for i in range(batch_size):
            # NOTE: remap the class id to coco category id
            label = torch.from_numpy(labels[i]).to(device)
            box = torch.from_numpy(boxes[i]).to(device)
            score = torch.from_numpy(scores[i]).to(device)
            from ..data.dataset import mscoco_label2category
            label = torch.tensor([mscoco_label2category[int(x.item())] for x in label.flatten()])\
                .to(box.device).reshape(label.shape)
