...
class BaseSolver(object):
    ...
    def _setup(self, ):
        ...
        """
        Add support to evalute TensorRT engine and onnx model
        """
        self.use_trl_engine = False
        from .trt_engine import TRTInference
        if trt_engine_path := cfg.yaml_cfg.get("trt_engine_path"):
            self.trt_engine = TRTInference(trt_engine_path, device=self.device)
            self.use_trl_engine = True


        import onnxruntime as ort
        from onnxruntime import InferenceSession
        self.use_onnx_model = False
        if onnx_model_path := cfg.yaml_cfg.get("onnx_model_path"):
            print(f"\033[91m Onnxruntime device:{self.device}\033[0m") 
            if self.device.type == 'cuda':
                self.onnx_model = InferenceSession(onnx_model_path, providers=['CUDAExecutionProvider'])
            else:
                self.onnx_model = InferenceSession(onnx_model_path, providers=['CPUExecutionProvider'])
