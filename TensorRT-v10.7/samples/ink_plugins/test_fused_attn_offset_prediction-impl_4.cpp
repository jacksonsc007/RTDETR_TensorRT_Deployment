...
NOTE:
1. Plugin attributes are allocated to GPU memory in fused_attn_offset_prediction::enqueue. Compared with
    implement-3, the gpu buffer for plugin attributes are local variables, instead of class members.

2. Note that memory alignment is indispensible, or the following error will occur:
    "pure virtual method called"
    "Cuda Runtime (misaligned address)"
3. Make sure getworkspace() grants enough memory, or cudamemcpy will fail.
...
private:
    ...
    Tactic tactic;
    bool is_attr_allocated = false;

    // float * input_scale_ptr_d; 
    // int8_t * input_offset_ptr_d;
    // float * fused_weight_scale_ptr_d;
    // int8_t * fused_weight_offset_ptr_d;
    // int8_t * fused_weight_ptr_d;
    // float * fused_weight_bias_ptr_d;
    // float * fused_output_ptr_d;
...
public:
    ...
    {
        // printf("\e[32m[Seting number tactics to 2]\e[m \n");
        return 2;
    }
    ...
    {
        printf("\e[32m[configurePlugin]\e[m \n");
        return 0;
    }
        ...
        void* const* outputs, void* workspace, cudaStream_t stream) noexcept override
    {
        ...
        */
        ...
        float * input_scale_ptr_d = reinterpret_cast<float*>(workspace_ptr);
        workspace_ptr += alignSize( sizeof(float) * input_scale.size() );
        int8_t * input_offset_ptr_d = reinterpret_cast<int8_t*>(workspace_ptr);
        workspace_ptr += alignSize( sizeof(int8_t) * input_offset.size() );
        float * fused_weight_scale_ptr_d = reinterpret_cast<float*>(workspace_ptr);
        workspace_ptr += alignSize( sizeof(float) * (attn_dim + sp_dim) );
        int8_t * fused_weight_offset_ptr_d = reinterpret_cast<int8_t*>(workspace_ptr);
        workspace_ptr += alignSize(sizeof(int8_t) * (attn_dim + sp_dim));
        int8_t * fused_weight_ptr_d = reinterpret_cast<int8_t*>(workspace_ptr);
        workspace_ptr += alignSize(sizeof(int8_t) * (attn_dim + sp_dim) * feature_dim);
        float * fused_weight_bias_ptr_d = reinterpret_cast<float*>(workspace_ptr);
        workspace_ptr += alignSize(sizeof(float) * (attn_dim + sp_dim));
        float * fused_output_ptr_d = reinterpret_cast<float*>(workspace_ptr);
        workspace_ptr += alignSize(sizeof(float) * batchsize * num_queries * (attn_dim + sp_dim));
        // cudaMemsetAsync(workspace_ptr, 0, sizeof(float) * batchsize * num_queries * (attn_dim + sp_dim) , stream);


        ASSERT(
            !cudaMemcpy(input_scale_ptr_d, &input_scale.front(), input_scale.size() * sizeof(float), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(input_offset_ptr_d, &input_offset.front(), input_offset.size() * sizeof(int8_t), cudaMemcpyHostToDevice)
        );

        ASSERT(
            !cudaMemcpy(fused_weight_scale_ptr_d, &attn_weight_scale.front(), attn_weight_scale.size() * sizeof(float), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_scale_ptr_d + attn_weight_scale.size(), &sp_weight_scale.front(), sp_weight_scale.size() * sizeof(float), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_offset_ptr_d, &attn_weight_offset.front(), attn_weight_offset.size() * sizeof(int8_t), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_offset_ptr_d + attn_weight_offset.size(), &sp_weight_offset.front(), sp_weight_offset.size() * sizeof(int8_t), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_ptr_d, &int8_attn_weight.front(), int8_attn_weight.size() * sizeof(int8_t), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_ptr_d + int8_attn_weight.size(), &int8_sp_weight.front(), int8_sp_weight.size() * sizeof(int8_t), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_bias_ptr_d, &attn_fp32_bias.front(), attn_fp32_bias.size() * sizeof(float), cudaMemcpyHostToDevice)
        );
        ASSERT(
            !cudaMemcpy(fused_weight_bias_ptr_d + attn_fp32_bias.size(), &sp_fp32_bias.front(), sp_fp32_bias.size() * sizeof(float), cudaMemcpyHostToDevice)
        );
        is_attr_allocated = true;
...
// #define DEBUG_INPUT
        ...
        fused_attn_offset_prediction_impl(
            ...
            input_scale_ptr_d,
            input_offset_ptr_d,
            fused_weight_scale_ptr_d,
            fused_weight_offset_ptr_d,
            fused_weight_ptr_d,
            fused_weight_bias_ptr_d,
            num_queries,
            output_col,
            n_channels,
            fused_output_ptr_d,
        ...
        ASSERT(
            !cudaMemcpy(outputs[0], fused_output_ptr_d, batchsize * num_queries * 96 * sizeof(float), cudaMemcpyDeviceToDevice)
        );
        ASSERT(
            !cudaMemcpy(outputs[1], fused_output_ptr_d + batchsize * num_queries * 96, batchsize * num_queries * 192 * sizeof(float), cudaMemcpyDeviceToDevice)
        );
...
// #define DEBUG_OUTPUT
        ...
        DynamicPluginTensorDesc const* outputs, int32_t nbOutputs) const noexcept override
    {
        ...
        // NOTE: We need to allocate enough space for the enqueue method to run.
...
int main(int argc, char** argv)
{
    ...
    printf("\e[32m =========== Start engine inference  =========== \e[m \n");
    if (!sample.infer())
    {
        return sample::gLogger.reportFail(sampleTest);
    }
