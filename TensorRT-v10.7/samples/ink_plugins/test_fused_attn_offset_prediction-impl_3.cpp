        ...
        void* const* outputs, void* workspace, cudaStream_t stream) noexcept override
    {
        ...
        */
        ...
        if (!is_attr_allocated)
        {
            ...
            input_scale_ptr_d = reinterpret_cast<float*>(workspace_ptr);
            workspace_ptr += alignSize( sizeof(float) * input_scale.size() );
            input_offset_ptr_d = reinterpret_cast<int8_t*>(workspace_ptr);
            workspace_ptr += alignSize( sizeof(int8_t) * input_offset.size() );
            fused_weight_scale_ptr_d = reinterpret_cast<float*>(workspace_ptr);
            workspace_ptr += alignSize( sizeof(float) * (attn_dim + sp_dim) );
            fused_weight_offset_ptr_d = reinterpret_cast<int8_t*>(workspace_ptr);
            workspace_ptr += alignSize(sizeof(int8_t) * (attn_dim + sp_dim));
            fused_weight_ptr_d = reinterpret_cast<int8_t*>(workspace_ptr);
            workspace_ptr += alignSize(sizeof(int8_t) * (attn_dim + sp_dim) * feature_dim);
            fused_weight_bias_ptr_d = reinterpret_cast<float*>(workspace_ptr);
            workspace_ptr += alignSize(sizeof(float) * (attn_dim + sp_dim));
            fused_output_ptr_d = reinterpret_cast<float*>(workspace_ptr);
